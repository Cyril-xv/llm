...
public class QuestionRewriteService {
    ...
    public List<String> decompose(String question) {
        log.info("问题分解，原始问题：{}",question);
        PromptTemplate realPrompt = new PromptTemplate(DECOMPOSE_PROMPT);
        realPrompt.add(QUESTION,question);
        String result = chatModel.call(realPrompt.create()).getResult().getOutput().getText();
    ...
    public String enrich(String chatHistory,String question){
        ...
        String text = chatModel.call(promptTemplate.create()).getResult().getOutput().getText();
        log.info("=====================问题富化完成，结果:{}=====================",text);
        return text;
    }
